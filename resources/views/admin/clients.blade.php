...
<div class="col-sm-10">
<textarea type="text" name="refer_link"  value="" class="form-control" placeholder="Refer Link"></textarea>
</div>
</div>
</br>
</br>
        ...
        <tbody>
          @foreach($clients as $index=>$client)
            <tr>
                <td>{{$index+1}}</td>
                <td>{{$client->name}}</td>
                <td><img src="{{img($client->picture)}}" width="50"></td>
                <td>{{$client->refer_link}}</td>
                <td>
                                    <a href="{{url('admin/clients/'.$client->id)}}" class="btn btn-info"><i class="fa fa-pencil"></i> Edit</a>
                                    <a href="{{url('admin/clients/delete/'.$client->id)}}"><button class="btn btn-danger" onclick="return confirm('Are you sure?')"><i class="fa fa-trash"></i> Delete</button></a>
                </td>
            </tr>
          @endforeach
        </tbody>
